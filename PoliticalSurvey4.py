...
#These are used to access the database in MongoDB.
cluster=MongoClient("mongodb+srv://Abhinav:t$85!2r8f7@hackuwu-mhcnt.mongodb.net/Survey?retryWrites=true&w=majority")
db=cluster['Survey']
...
def user_indentification():
    ...
    while True:
        try:
            ...
            print(collection.find_one({'_id:' : ssn}))
            break
        except:
            print("Invalid input.")
...
def add_to_database():
    global economic, diplomatic, civil, societal
    ssn=int(input("enter num: "))
    post={
          'Economic Points (<50pts favors market, >50pts favors equality, =50 points favors none) ' : economic,
          'Diplomatic Points (<50pts favors world, >50pts favors nation, =50 points favors none) ' : diplomatic,
          'Civil Points (<50pts favors authority, >50pts favors liberty, =50 points favors none) ': civil,
          'Societal Points (<50pts favors progress, >50pts favors tradition, =50 points favors none) ': societal}
    collection.insert_one(post)
    post_id=posts.insert_one(post).inserted_id
    print(post_id)
#This function will purge the database. Call it to delete everything from the database. This function is only for debugging purposes.
def delete_database():
